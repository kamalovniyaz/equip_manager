    ...
    <div class="form-columns">
      <div class="form-column">
        <div class="form-group">
          <select v-model="equipmentType" class="form-input" required>
            <option value="" disabled selected>Выберите тип оборудования</option>
            <option v-for="type in equipmentTypes" :key="type.id">
              {{ type['name'] }}
            </option>
          </select>
        </div>
      ...
      <button class="form-button" @click="addEquipment">Создать</button>
  ...
  methods: {
    ...
    addEquipment() {
      ...
      axios
          ...
          .then(() => {
            this.successToast('Оборудование успешно сохранено.')
            setTimeout(() => {
              this.clearForm()
